			...
			$image = sprintf(
				'<div class="et_pb_team_member_image et-waypoint%3$s">
					...
					<div class="footer-img">
						<div class="name-persona">%4$s</div>
						<div class="email-persona">%5$s</div>
						<div class="social-persona">%6$s</div>
					</div>	
				</div>',
				esc_url( $image_url ),
				esc_attr( $name ),
								...
								if ( 'on' === $args['show_content'] ) {
									...
									if ( has_excerpt() ) {
										...
										if ( '' !== $post_content ) {
											...
											echo wpautop( truncate_post( 70, false, '', true ) );
					...
					if ( 'on' === $show_content ) {
						...
						if ( has_excerpt() ) {
							...
							if('off' === $fullwidth) {
								...
								echo wpautop( truncate_post( 370, false, '', true ) );
							}
						}
					}

					if ( 'on' !== $show_content ) {
						$more = 'on' == $show_more ? sprintf( ' <a href="%1$s" class="more-link" >%2$s</a>' , esc_url( get_permalink() ), esc_html__( 'Leia +', 'et_builder' ) )  : '';
						echo $more;
					}
	...
	<?php
		...
		$output = sprintf(
			'<div%5$s class="%1$s%3$s%6$s"%7$s>
				%2$s
			%4$s',
			( 'on' === $fullwidth ? 'et_pb_posts post-fullwidth' : 'et_pb_blog_grid clearfix' ),
			$posts,
			esc_attr( $class ),
